    ...
    <Flex gap={{ base: '2.5', sm: '3' }} flexWrap="wrap" align="center">
      {BADGES.map(({ icon, label, stars }) => (
        <Flex
          ...
          _hover={{ bg: 'rgba(108,99,255,0.15)', borderColor: 'rgba(108,99,255,0.35)' }}
          transition="all 0.2s"
          cursor="default"
        >
          ...
          {stars ? (
            <HStack gap="1" display="inline-flex">
              <Text as="span" color="yellow.300" fontSize="2xs" letterSpacing="tight">★★★★★</Text>
              <Text fontSize="xs" fontWeight="medium" color="gray.300">{label}</Text>
            </HStack>
          ) : (
            <Text fontSize="xs" fontWeight="medium" color="gray.300">{label}</Text>
          )}
